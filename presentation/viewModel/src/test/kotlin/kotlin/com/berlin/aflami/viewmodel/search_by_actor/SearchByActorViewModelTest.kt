...
class SearchByActorViewModelTest {
    ...
    fun `when onActorNameChanged should update actorName in uiState`() = runTest {
        ...
        // Then
        val uiState = viewModel.searchByActorUiState.first()
    ...
    fun `when onSearchClick should set isLoading to true initially`() = runTest {
        ...
        val actorName = "Tom"
        viewModel.onActorNameChanged(actorName)
        coEvery { searchByActorNameUseCase(actorName) } returns emptyList()

        // When
        viewModel.onSearchClick()
        val initialUiState = viewModel.searchByActorUiState.first()
    ...
    fun `when onSearchClick should update uiState with error on failure`() = runTest {
        ...
        coEvery { searchByActorNameUseCase(actorName) } throws Exception(errorMessage)
        viewModel.onActorNameChanged(actorName)
        ...
        assertThat(uiState.media).isEmpty()
        coVerify(exactly = 1) { searchByActorNameUseCase(actorName) }
    }
}